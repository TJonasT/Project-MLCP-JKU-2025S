...
class BiLSTMClassifier(nn.Module):
    """
    Bidirectional LSTM classifier with a linear output layer.
...
if __name__ == '__main__':
    ...
    with open(f"LSTM/lstm_{hparams['hidden_dim']}x{hparams['num_layers']}_hparams.pkl", "wb") as file:
    ...
    checkpoint_cb = ModelCheckpoint(
        ...
        filename=f"lstm_{hparams['hidden_dim']}x{hparams['num_layers']}",
        dirpath="LSTM"
    )
    ...
    wandb_logger = WandbLogger(
        ...
        name=f"BiLSTM-{hparams['hidden_dim']}x{hparams['num_layers']}",
        config=hparams
    )
    ...
    lstm_result = test_results[0]
    lstm_costs = {
        cls: lstm_result[f"test/cost/{cls}"]
        for cls in TARGET_CLASSES
        if f"test/cost/{cls}" in lstm_result
    }

    # Add total cost
    lstm_costs["TOTAL"] = lstm_result["test/total_cost"]
    ...
    print(tabulate(cost_df.reset_index().values,
                   headers=["Class", "Baseline", "Logistic Regression", "LSTM"],
                   tablefmt="github"))

    with open(f"LSTM/lstm_{hparams['hidden_dim']}x{hparams['num_layers']}_costs.pkl", "wb") as file:
        pickle.dump(lstm_costs, file)
